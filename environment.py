...
class GLMBandit:
    def __init__(self, name=None, config=None, load=None):
        if load is None:
            ...
            self.full_norm = config.get('full_norm', False) 
            ...
            self.kappa_star = self.calculate_kappa_star()
            self.t = 0
    ...
    def calculate_kappa_star(self):
        max_mu_dot = -np.inf
        print('Calculating kappa star...')
        for arm_set in tqdm(self.arms):
            X_mat = np.array(arm_set)
            dotp = np.dot(X_mat, self.theta)
            if self.model == 'Logistic':
                ...
                mu_dot = dprobit(dotp)
            max_mu_dot = max_mu_dot if np.max(mu_dot) < max_mu_dot else np.max(mu_dot)
        return 1/max_mu_dot
    ...
    def step(self, action):
        # Helper function to compute reward for a single action
        def compute_single_reward(action_idx):
            try:
                dot_product = np.dot(self.theta, self.arms[self.t][action_idx])
                
                if self.model == 'Logistic':
                    reward = sigmoid(dot_product)
                    # Handle edge cases for binomial sampling
                    reward = max(0.0, min(1.0, reward))  # Clamp to [0,1]
                    noisy_reward = float(self.rng.binomial(1, reward))
                elif self.model == 'Probit':
                    reward = probit(dot_product)
                    noisy_reward = float(self.rng.normal(dot_product) > 0.0)
                else:
                    raise ValueError(f"Unknown model: {self.model}")
                
                regret = self.max_reward[self.t] - reward
                return noisy_reward, regret
                
            except Exception as e:
                ##print(f"Error processing action {action_idx}: {e}")
        ...
        for a in action:
            noisy_reward, regret = compute_single_reward(a)
